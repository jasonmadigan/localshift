...
import (
	...
	"github.com/jasonmadigan/localshift/pkg/cluster"
...
const (
	containerName = "localshift"
	hostname      = "127.0.0.1.nip.io"
	labelKey      = "io.localshift.cluster"
...
func Create(opts CreateOpts, logger *slog.Logger) error {
	...
	if rt.ContainerExists(containerName) {
		...
		if opts.DisableOverlayCache {
			copts.Volumes = append(copts.Volumes, "localshift-storage:/host-container")
		} else {
	...
	// orbstack/cri-o fix: remove read-only host-container store if present
...
// on OrbStack where it's mounted read-only and breaks pod scheduling.
func fixCRIOStorage(rt *runtime.Runtime, container string, logger *slog.Logger) {
	...
	logger.Info("fixing CRI-O storage config for OrbStack")

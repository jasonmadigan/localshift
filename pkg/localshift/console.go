...
const (
	...
	consoleContainer = "localshift-console"
)
...
func applyConsolePluginCRD(dynClient dynamic.Interface, ver version.OCPVersion) error {
	url := ver.ConsolePluginCRDURL()

	resp, err := http.Get(url)
	if err != nil {
		return fmt.Errorf("fetching CRD from %s: %w", url, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		return fmt.Errorf("fetching CRD: HTTP %d from %s", resp.StatusCode, url)
	}
...
func createConsoleRBAC(client kubernetes.Interface) error {
	...
	binding := &rbacv1.ClusterRoleBinding{
		ObjectMeta: metav1.ObjectMeta{Name: "localshift-console-admin"},
	...
	impRole := &rbacv1.ClusterRole{
		ObjectMeta: metav1.ObjectMeta{Name: "localshift-console-impersonator"},

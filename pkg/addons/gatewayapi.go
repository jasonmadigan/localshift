...
//go:embed manifests/gateway-api.yaml
...
func (g *gatewayAPI) Install(ctx context.Context, cfg *Config) error {
	data, err := manifests.ReadFile("manifests/gateway-api.yaml")
	if err != nil {
		return fmt.Errorf("reading embedded gateway-api manifests: %w", err)
	}
